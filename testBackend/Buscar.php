<?php
  function Buscar($tipo,$form)
  {
	...
	$sql = mysql_query("SELECT a.id_trayectos, a.fecha_salida, a.duracion, b.nombre AS ciudad_origen, c.nombre AS ciudad_destino, e.denominacion FROM prueba_julian.trayectos AS a INNER JOIN prueba_julian.ciudades AS b ON a.ciudad_origen_id=b.id INNER JOIN prueba_julian.ciudades AS c ON a.ciudad_destino_id=c.id INNER JOIN prueba_julian.categorias_trayectos AS d ON a.id_trayectos=d.trayectos_id INNER JOIN prueba_julian.categorias AS e ON d.categorias_id=e.id ".$add_sql."ORDER BY a.id_trayectos");
      ...
      while ($row=mysql_fetch_array($sql))
      {
		...
		else
		{
		  ...
		  $trayecto .= '      <div class="panel-body jfrrtbldes2">
							    <table id="tabla_u" class="display" cellspacing="0" width="90%">
							      <thead>
							        <tr>
							          <th class="jfrrtblbrd" style="text-align: center">Fecha y Hora Salida</th>
							          <th class="jfrrtblbrd" style="text-align: center">Ciudad Origen</th>
